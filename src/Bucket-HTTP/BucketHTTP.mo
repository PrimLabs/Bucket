...
module {
    ...
    public class BucketHTTP() {
        ...
        private type HttpRequest        = Types.HttpRequest;
        private type HttpResponse       = Types.HttpResponse;
        private type CallbackToken      = Types.CallbackToken;
        private type StreamingCallback  = query (CallbackToken) -> async (StreamingCallbackHttpResponse);
        private type StreamingCallbackHttpResponse = Types.StreamingCallbackHttpResponse;
        ...
        private var streamingCallback : ?StreamingCallback = null; 
        ...
        // // 流复调用
        // public func streamingCallback(token: CallbackToken) : StreamingCallbackHttpResponse {
        //     let g: GET = {
        //         file_key = token.key;
        //         flag     = token.index;
        //     };
        ...
        //     switch (_get(g)) {
        //         case (#ok(payload)) {
        //             let newtoken: CallbackToken = {
        //                 index     = token.index + 1;
        //                 max_index = token.max_index;
        //                 key       = token.key;
        //             };
        //             return {
        //                 body  = payload;
        //                 token = if (token.index == token.max_index) { null } else { ?newtoken };
        //             }
        //         };
        //         case (#err(_err)) {
        //             return {
        //                 body  = Text.encodeUtf8("StreamingCallack Wrong!");
        //                 token = null;
        //             }
        //         };
        //     };
        ...
        public func http_request(request: HttpRequest) : HttpResponse {
            ...
            if (path.size() != 0 and path.size() == 2 and path[0] ==  "fk") {
                let file_key : Text = path[1]; 
                var flag: Nat = 0;
                switch (asset_map.get(file_key)) {
                    case (null) { return errStaticPage() };
                    case (?asset) { flag := asset.page_field.size() };
                };
                if (flag == 0) { // 只有一片数据
                    let g: GET = {
                        file_key = file_key;
                    ...
                    switch(_get(g)) {
                        case (#ok(payload)) {
                            return {
                                status_code = 200;
                                headers     = [("Content-Type", "text/plain; version=0.0.4"), ("Content-Length", Nat.toText(payload.size()))];
                                body        = payload;
                                streaming_strategy = null;
                            }
                        };
                        case (#err(err)) { return errStaticPage() };
                    };
                } else { // 多片数据
                    let g: GET = {
                        file_key = file_key;
                        ...
                        case(#ok(payload)) {
                            ...
                            return {
                                ...
                                streaming_strategy = ?#Callback({
                                    callback = switch(streamingCallback) {
                                        case(null) {return errStaticPage();};
                                        case(?scb) {scb};
                                    };
                                    token    = newtoken;
